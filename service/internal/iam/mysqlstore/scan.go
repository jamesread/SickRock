...
func userAccountSelectSQL() string {
	return `SELECT id, username, password_hash, created_by, COALESCE(created_at, ''), COALESCE(updated_at, '') FROM user_accounts`
}
...
func sessionSelectSQL() string {
	return `SELECT id, sid, user_account_id, impersonator_user_id, COALESCE(created_at, ''), COALESCE(updated_at, '') FROM sessions`
}
...
func apiKeySelectSQL() string {
	return `SELECT id, user_account_id, name, key_value, read_only, COALESCE(last_used_at, ''), COALESCE(created_at, ''), COALESCE(updated_at, '') FROM api_keys`
}
...
func (s *MySQL) queryStrings(ctx context.Context, q string, args ...any) ([]string, error) {
...
func (s *MySQL) queryInts(ctx context.Context, q string, args ...any) ([]int, error) {
	rows, err := s.db.QueryContext(ctx, q, args...)
	if err != nil {
		return nil, err
	}
