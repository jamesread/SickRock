...
func (s *MySQL) CreateSession(ctx context.Context, sid string, userID int, impersonatorID *int) error {
	var impersonator any
	if impersonatorID != nil {
		impersonator = *impersonatorID
	}
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO sessions (sid, user_account_id, impersonator_user_id, created_at, updated_at) VALUES (?, ?, ?, NOW(3), NOW(3))`,
...
func (s *MySQL) GetSessionBySID(ctx context.Context, sid string) (*store.SessionRow, error) {
...
func (s *MySQL) DeleteSession(ctx context.Context, sid string) error {
	return s.execOp(ctx, "delete session", `DELETE FROM sessions WHERE sid = ?`, sid)
}

func (s *MySQL) DeleteSessionsForUser(ctx context.Context, userID int) error {
