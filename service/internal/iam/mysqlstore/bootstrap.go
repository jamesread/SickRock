...
func (s *MySQL) EnsureRBACBootstrap(ctx context.Context) error {
...
func bootstrapGroups(ctx context.Context, tx *sql.Tx) error {
	_, err := tx.ExecContext(ctx,
		`INSERT IGNORE INTO user_groups (name) VALUES (?), (?)`,
...
func bootstrapFirstSuperuser(ctx context.Context, tx *sql.Tx) error {
	...
	_, err := tx.ExecContext(ctx,
		`INSERT IGNORE INTO user_group_memberships (user_account_id, user_group_id)
...
func (s *MySQL) EnsureUserInEveryoneGroup(ctx context.Context, userID int) error {
	return s.EnsureUserInGroup(ctx, userID, rbac.GroupEveryone)
}

func (s *MySQL) EnsureUserInGroup(ctx context.Context, userID int, groupName string) error {
...
func (s *MySQL) CountUsersWithSuperuserViaGroups(ctx context.Context) (int, error) {
	var n int
...
func (s *MySQL) ensureSuperuserCoverage(ctx context.Context) error {
