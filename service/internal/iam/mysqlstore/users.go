...
func (s *MySQL) CountUserAccounts(ctx context.Context) (int, error) {
	var n int
...
func (s *MySQL) GetUserByUsername(ctx context.Context, username string) (*store.UserAccountRow, error) {
	return s.getUser(ctx, userAccountSelectSQL()+` WHERE username = ?`, username)
}

func (s *MySQL) GetUserByID(ctx context.Context, id int) (*store.UserAccountRow, error) {
	return s.getUser(ctx, userAccountSelectSQL()+` WHERE id = ?`, id)
}

func (s *MySQL) getUser(ctx context.Context, q string, arg any) (*store.UserAccountRow, error) {
...
func (s *MySQL) ListUserAccounts(ctx context.Context) ([]store.UserAccountRow, error) {
	rows, err := s.db.QueryContext(ctx, userAccountSelectSQL()+` ORDER BY username`)
	if err != nil {
...
func (s *MySQL) CreateUserAccount(ctx context.Context, username, passwordHash, createdBy string) (int, error) {
	res, err := s.db.ExecContext(ctx,
		`INSERT INTO user_accounts (username, password_hash, created_by, created_at, updated_at) VALUES (?, ?, ?, NOW(3), NOW(3))`,
...
func (s *MySQL) DeleteUserAccount(ctx context.Context, id int) error {
	return s.execOp(ctx, "delete user account", `DELETE FROM user_accounts WHERE id = ?`, id)
}

func (s *MySQL) UpdateUserPassword(ctx context.Context, id int, passwordHash string) error {
	return s.execOp(ctx, "update user password",
		`UPDATE user_accounts SET password_hash = ?, updated_at = NOW(3) WHERE id = ?`,
		passwordHash, id)
}
...
func (s *MySQL) execOp(ctx context.Context, op, q string, args ...any) error {
